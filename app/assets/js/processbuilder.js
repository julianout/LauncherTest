...
class ProcessBuilder {
    ...
    _resolveModuleLibraries(mdl){
        ...
        for(let sm of mdl.subModules){
            ...
            // Handle VersionManifest submodules - parse JSON and extract libraries
            if(sm.rawModule.type === Type.VersionManifest){
                try {
                    const versionData = this._loadVersionManifest(sm.getPath())
                    if(versionData && versionData.libraries){
                        logger.info('[NeoForge]: Processing', versionData.libraries.length, 'libraries from version manifest')
                        for(let lib of versionData.libraries){
                    ...
                    logger.warn('[NeoForge]: Failed to process version manifest:', err.message)
                }
            }
    ...
    /**
     * Load and parse a version manifest JSON file
     * @param {string} manifestPath Path to the manifest file
     * @returns {Object|null} Parsed manifest data or null if failed
     */
    _loadVersionManifest(manifestPath) {
        try {
            if (fs.existsSync(manifestPath)) {
                const manifestData = fs.readFileSync(manifestPath, 'utf8')
                return JSON.parse(manifestData)
            } else {
                logger.warn('[NeoForge]: Version manifest not found:', manifestPath)
                return null
            }
        } catch (err) {
            logger.error('[NeoForge]: Failed to load version manifest:', err.message)
            return null
        }
    }

    /**
     * Download a library from version manifest if needed
     * @param {Object} lib Library object from version manifest
     * @returns {string|null} Path to downloaded library or null if failed
     */
    _downloadVersionManifestLibrary(lib) {
        try {
            const artifact = lib.downloads.artifact
            ...
            if (fs.existsSync(libPath)) {
                ...
                logger.debug('[NeoForge]: Library already exists:', lib.name)
                return libPath
            }
            ...
            logger.info('[NeoForge]: Downloading library:', lib.name)
            
            // For now, return expected path - actual download will be handled by Helios core
            // This tells the classpath builder where to expect the file
            return libPath
            
        } catch (err) {
            logger.error('[NeoForge]: Failed to process library:', lib.name, err.message)
            return null
        }
    }
